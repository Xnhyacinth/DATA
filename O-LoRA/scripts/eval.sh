...
for part in "${parts[@]}"; do
   ...
   if [ "$part" == "$last_element" ];then
      flag=0
      model=${output_prefix}/${order_type}/outputs/${i}-${part}/${weights}
   fi
   if [ "$flag" == "1" ];then
      continue
   fi
   echo "model: ${model}"
   echo "output_dir: ${output_prefix}_eval/${order_type}/outputs/${i}-${part}"
   ...
   deepspeed --master_port $port --include localhost:${gpus} ${python_file} \
      ...
      --output_dir ${output_prefix}_eval/${order_type}/outputs/${i}-${part} \
