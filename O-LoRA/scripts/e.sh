...
for ini_threshold in "${ini_thresholds[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    #     echo "Log file already exists: $LOGFILE"
    # fi
    mkdir -p ${output_prefix}_eval/order4/logs
    LOGFILE="${output_prefix}_eval/order4/logs/train_and_infer_${ini_threshold}.log"
    bash scripts/eval.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order4 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
    # else
    #     echo "Log file already exists: $LOGFILE"
    # fi

    mkdir -p ${output_prefix}_eval/order5/logs
    LOGFILE="${output_prefix}_eval/order5/logs/train_and_infer_${ini_threshold}.log"
    # if [ ! -f "$LOGFILE" ]; then
    bash scripts/eval.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order5 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
    # else
    #     echo "Log file already exists: $LOGFILE"
    # fi

    mkdir -p ${output_prefix}_eval/order6/logs
    LOGFILE="${output_prefix}_eval/order6/logs/train_and_infer_${ini_threshold}.log"
    # if [ ! -f "$LOGFILE" ]; then
    bash scripts/eval.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order6 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
