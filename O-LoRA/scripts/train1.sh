...
for ini_threshold in "${ini_thresholds[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    bash scripts/order2.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order4 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
    # else
    #     echo "Log file already exists: $LOGFILE"
    # fi
    ...
    # if [ ! -f "$LOGFILE" ]; then
    bash scripts/order2.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order5 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
    # else
    #     echo "Log file already exists: $LOGFILE"
    # fi
    ...
    # if [ ! -f "$LOGFILE" ]; then
    bash scripts/order2.sh ${model} ${tuning_method} ${method} ${ini_threshold} ${seed} ${gpus} order6 ${lr} ${lr_type} ${output_prefix} > "$LOGFILE" 2>&1
