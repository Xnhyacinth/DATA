...
class T5DenseActDense(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if hasattr(self.wo, 'linear_data'):
            if (
                isinstance(self.wo.linear_data.weight, torch.Tensor)
                and hidden_states.dtype != self.wo.linear_data.weight.dtype
                and self.wo.linear_data.weight.dtype != torch.int8
            ):
                hidden_states = hidden_states.to(self.wo.linear_data.weight.dtype)
        else:
            if (
    ...
    def prompt_init(self):
        ...
        if 'adaprompt' in self.config.to_dict().keys() and self.config.adaprompt:
            ...
            # setattr(self, f'e_a_{e}',a)
                ...
                setattr(self, f'data_k_{e}', k)
                setattr(self, f'data_a_{e}', a)
                setattr(self, f'data_w_{e}', w)
                setattr(self, f'data_k2_{e}', k2)
                setattr(self, f'data_a2_{e}', a2)
                setattr(self, f'data_w2_{e}', w2)
                if self.config.scale_bakebone:
                    ...
                    setattr(self, f'data_k3_{e}', k3)
                    setattr(self, f'data_a3_{e}', a3)
                    setattr(self, f'data_w3_{e}', w3)
...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs, ortho_loss_en = self.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,
                head_mask=head_mask,
        ...
        # Decode
        decoder_outputs, ortho_loss_de = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            inputs_embeds=decoder_inputs_embeds,
            past_key_values=past_key_values,
