        ...
        if "adaprompt" in self.config.to_dict().keys() and self.config.adaprompt:
            ...
            for e in range(self.config.num_hidden_layers // self.config.gap_layers):
                ...
                if self.config.scale_bakebone:
                    ...
                    logger.info("adaprompt scale bakebone!")
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            if "adaprompt" in self.config.to_dict().keys() and self.config.adaprompt:
                if layer_idx % self.config.gap_layers == 0:
                    e = layer_idx // self.config.gap_layers
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def prompt_init(self):
        return self.language_model.prompt_init()

    def post_prompt_init(self):
        return self.language_model.post_prompt_init()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs, ortho_loss = self.language_model(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if hasattr(self.config.text_config, "ortho_mu") and self.config.text_config.ortho_mu and self.training:
            loss += ortho_loss
