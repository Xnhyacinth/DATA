...
class FinetuningArguments(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "cl"] = field(
        default="sft",
        metadata={"help": "Which stage will be performed in training."},
    )
    finetuning_type: Literal["lora", "oft", "freeze", "full", "data"] = field(
        default="lora",
    ...
    scale: int | None = field(
        default=None,
        metadata={"help": "Whether or not to enable backbone scaling."},
    )
    unc_thr: float = field(
        default=0.2,
        metadata={"help": "Uncertainty threshold used by continual learning."},
    )
    ema_data: float = field(
        default=0.99,
        metadata={"help": "EMA decay for DATA teacher updates."},
    )
    ema_teacher: float = field(
        default=0.99,
        metadata={"help": "EMA decay for teacher model updates."},
    )
    adaprompt: int | None = field(
        default=None,
        metadata={"help": "Whether or not to enable AdaPrompt in DATA training."},
    )
    task_id: int | None = field(
        default=None,
        metadata={"help": "Current continual-learning task id."},
    )
    gap_layers: int = field(
        default=4,
        metadata={"help": "Layer interval used by AdaPrompt/DATA prompt injection."},
    )
    reinit: bool = field(
        default=False,
        metadata={"help": "Whether or not to re-initialize prompts when switching tasks."},
    )
    ortho_mu: float | None = field(
        default=None,
        metadata={"help": "Orthogonality loss weight for prompt tuning."},
    )
    scale_bakebone: bool | None = field(
        default=None,
        metadata={"help": "Whether or not to scale the backbone during DATA injection."},
    )
    nomlp: bool | None = field(
        default=None,
        metadata={"help": "Whether or not to skip MLP modules in DATA injection."},
    )
    cl_project: int | None = field(
        default=None,
        metadata={
            "help": (
                "Projection length used by continual-learning variants. "
                "Use `--cl_project` to avoid conflicting with TrainingArguments `--project`."
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        assert self.finetuning_type in ["lora", "oft", "freeze", "full", "data"], "Invalid fine-tuning method."
